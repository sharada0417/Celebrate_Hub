...
function PlaceListing() {
  ...
  const filteredHotels =
    selectedLocation === "ALL"
      ? celebratePlaces
      : celebratePlaces.filter((places) =>
          places.location.toLowerCase().includes(selectedLocation.toLowerCase())
        );
    ...
    <>
      <section className="px-8 py-8 lg:py-2 ">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-8 mt-4">
          {filteredHotels.map((places) => (
            <PlaceCard key={places._id} places={places} />
          ))}
        </div>
      </section>
    </>
